...
def main():
  ...
  print "loading data.."
  traindata = list(nmpy.array(pnds.read_table('train.tsv'))[:,2])
  ...
  #print predTrain[1]
  #print predTest[2]
  print "10 Fold CV Score: ", nmpy.mean(cross_validation.cross_val_score(rd2, predTrain, y, cv=10, scoring='roc_auc'))
  ...
  #testfile = testfile
  pred_df = pnds.DataFrame(predFinal, index=testfile.index, columns=['label'])
  #predTrain_df = pnds.DataFrame(predTrain, index=trainfile.index, columns=['label'])
  pred_df.to_csv('testTfidf.csv')
  #predTrain_df.to_csv('trainfilerainTfidf.csv')
